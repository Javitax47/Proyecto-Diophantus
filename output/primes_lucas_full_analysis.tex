...
\begin{align*}
& (RET - (0))^{2} + (n[t+1] - (n))^{2} + (result[t+1] - (0))^{2} = 0
\end{align*}\part{Fórmulas Generadoras}
\begin{small}
\begin{align*}
G &= 1 - ((RET - (0))^{2} + (n[t+1] - (n))^{2} + (result[t+1] - (0))^{2}) \\
\end{align*}
...
\mathcal{D}_{sym} &= (\mathit{RET} - (0))^2 + (\mathit{n} - (\mathit{n}))^2 + (R - (0))^2
\end{dmath*}\newpage
\section{Representación Operacional}
\begin{dmath*}
f(N) &= \sum_{R=0}^{\infty} R \cdot \left\lfloor \frac{1}{1 + \mathcal{D}(N, R, \mathbf{x})} \right\rfloor \\
\mathcal{D}(N, R, \mathbf{x}) &= \sum_{\mathbf{x} \in \mathbb{N}^{2}} \left( (x_{1} - (0))^2 + (x_{2} - (x_{2}))^2 + (R - (0))^2 \right)
\end{dmath*}
Vector de estado: $\mathbf{x} \in \mathbb{R}^{2}$ (variables internas anonimizadas).\\\part{Sistema Dinámico (Recurrencia Infinita)}
\begin{align*}
