...
\begin{align*}
cost[t+1] &= ('*', 'num_tokens', 1000000000000000000) \\
num\_tokens[t+1] &= num_tokens \\
\end{align*}\part{Conversión a Polinomio}
\subsection*{Ecuación Unificada P=0}
\begin{align*}
& (cost[t+1] - (num_tokens  \cdot  1000000000000000000))^{2} \\
& + (num_tokens[t+1] - (num_tokens))^{2} = 0
\end{align*}\part{Fórmulas Generadoras}
\begin{small}
\begin{align*}
G &= 1 - ((cost[t+1] - (num_tokens  \cdot  1000000000000000000))^{2} \\
& + (num_tokens[t+1] - (num_tokens))^{2}) \\
\end{align*}
...
\mathcal{D}_{sym} &= (\mathit{cost} - (\mathit{num\_tokens}  \\mathit{cdot}  1000000000000000000))^2 + \\
&  (\mathit{num\_tokens} - (\mathit{num\_tokens}))^2
\end{dmath*}\newpage
\section{Representación Operacional}
\begin{dmath*}
f(N) &= \sum_{R=0}^{\infty} R \cdot \left\lfloor \frac{1}{1 + \mathcal{D}(N, R, \mathbf{x})} \right\rfloor \\
\mathcal{D}(N, R, \mathbf{x}) &= \sum_{\mathbf{x} \in \mathbb{N}^{3}} \left( (x_{2} - (x_{1}  \x_{3}  1000000000000000000))^2 + (x_{1} - (x_{1}))^2 \right)
\end{dmath*}
Vector de estado: $\mathbf{x} \in \mathbb{R}^{3}$ (variables internas anonimizadas).\\\part{Sistema Dinámico (Recurrencia Infinita)}
\begin{align*}
