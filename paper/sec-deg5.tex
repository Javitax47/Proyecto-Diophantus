...
We start from the system of \cite{jsww1976}. It has $14$ equations in the
parameter $k$ and $25$ unknowns $a,b,\dots,z$; the primes are the values $k+2$
for which it is solvable, and by Proposition~\ref{prop:gen} it yields a
generator with $\nu=26$ and $\delta=1+2\cdot 12=25$. We display it once, with
its own numbering \textup{(S1)--(S14)}, and refer to those tags throughout;
this transcription is the one our formal development uses, and the displayed
\LaTeX{} is generated from it.
\label{ssec:sistema1}
\input{sys-original}
...
\begin{theorem}
\label{thm:deg5}
For every $k\ge0$, the system \textup{(S1)--(S14)} has a solution in
nonnegative integers if and only if the system displayed in
\S\ref{ssec:flat-system}---in $44$ variables, with every equation of degree at
most $2$---does. Consequently
\[
...
\begin{proposition}
\label{prop:mu20}
Let $a,r,y,u\in\Z$ with $a\ge 2$ and $r,y\ge 0$ satisfy \textup{(S7)}, namely
$16r^2y^4(a^2-1)+1=u^2$. Then $a+u^2(u^2-a)\ge 1$.
\end{proposition}
...
The hypothesis $a\ge 2$ is not an assumption: it follows from
\textup{(S3)}, \textup{(S4)} and \textup{(S5)} alone, independently of what the
system represents (Section~\ref{sec:lean}). Proposition~\ref{prop:mu20} is
formalized in Lean;
