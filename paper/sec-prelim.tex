...
\begin{remark}
Proposition~\ref{prop:gen} has a consequence we use as a consistency check
throughout: \emph{the degree of a generator is always odd}. Several figures in
the secondary literature are quoted without saying whether they refer to a
representation or to a generator; parity settles it.
\end{remark}
...
for $u$ and deleting that equation. Then $\Sigma$ and $\Sigma'$ are
equisatisfiable \emph{if and only if} every solution of $\Sigma'$ satisfies
$E\ge 0$.
\end{proposition}

\begin{proof}
Forward: a solution of $\Sigma$ restricts to one of $\Sigma'$. Backward: given a
solution of $\Sigma'$, setting $u:=E$ satisfies $\Sigma$ provided $E\in\N$,
i.e.\ $E\ge 0$; if $E<0$ for some solution of $\Sigma'$, that solution is
spurious.
\end{proof}
